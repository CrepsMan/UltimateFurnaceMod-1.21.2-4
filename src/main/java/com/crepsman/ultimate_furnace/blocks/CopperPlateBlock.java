...
public class CopperPlateBlock extends Block implements Waterloggable {
	...
	protected void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
		if (state.get(HOT) && world instanceof ServerWorld serverWorld) {
			...
			entity.damage(serverWorld.getDamageSources().inFire(), 2.0F); // Fixed parameter count
	...
	public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
		if (neighborState.getFluidState().getFluid() == Fluids.WATER ||
			neighborState.getFluidState().getFluid() == Fluids.FLOWING_WATER ||
			(neighborState.contains(Properties.WATERLOGGED) && neighborState.get(WATERLOGGED)) ||
			neighborState.isIn(COLD_BLOCK_TAG)) {
			if (world instanceof World worldInstance) {
				worldInstance.scheduleBlockTick(pos, this, 3);
			}
		}
